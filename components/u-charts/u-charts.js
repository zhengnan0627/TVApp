...
var config = {
  ...
  colors: ['#facc14', '#1890ff', '#90ed7d', '#f04864', '#8543e0', '#90ed7d'],
...
function drawPieText(series, opts, config, context, radius, center) {
  ...
  var seriesConvert = series.map(function(item) {
    // var text = item.format ? item.format(+item._proportion_.toFixed(2)) : +item._proportion_.toFixed(2);
	var text = item.format ? item.format(+item._proportion_.toFixed(2)) : item.data;
  ...
  for (let i = 0; i < textObjectCollection.length; i++) {
    ...
    context.setFillStyle(item.textColor || '#FFFFFF');
